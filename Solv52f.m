...
% ______ Исп-ся в fzero(dva)
function X = Solv52f(KZ,nT,n,c,dva,io,h,L1, q,x10,x20,Xot,Lot)
...
while i <= q 
  ...
  if ischar(x0),  X = ['Solv52f-KIW2B-KIN1B ' x0];  return,end 
  ...
  if FL == 1             % К след(i+1-му) корню  
     ...
     X = sprintf(['Solv52f: Корень не найден\nio=%d i=%d\n' ...  
         'x0=[%3.17g  %3.17g]\nf =[%3.17g, %3.17g\nFL=%d %s]'],...
