...
% ______ Исп-ся в  XRS. Возм переход через Т (io:1->2)
function X = Solv52T( nT, n,c, io, h,L1, q, x01,f1,x02,f2, ...
                      xE1,xE2, Xot, Lot, e1v,e3v,V2v,T2v ) 
% xEi - Е-корень или прибл Е-КИН для Li
% xHi - H-КИН, или прибл Н-КИН или Ve(точн корень) или 'НеМБ' для Li
% fHi - 0,1,2,3,4 или HF(KИН)   ТСВ4-83
 
%global qL5 xL5  LL5  % Впервые генерируются п-ры лунки
%global TFL  infC  % stat  vhea
...
while i <= q 
  ...
  if io_1  &&  x0e*x0e > T2
     if x0(1)^2 < T2,  x0(1) = [x0e; x0e+AP2]; end 
     io_1 = false;  
  ...
  if ischar(x0),  X = ['Solv52T-KIW2B-KIN1B ' x0];  return,end   
  ...
  if FL == 1                 % К след(i+1-му) корню  
     ...
     x0ep = x0e;                    
     Lp   = Lp+h;
     %if XEnw, XEnw=false; qL5=qL5+1;  xL5(qL5)=Xp-XE;  LL5(qL5)=Lp;  end
  else                     % Выход   
      X = sprintf(['Solv52T: Корень не найден\nio=%d i=%d\n' ...  
         'x0=[%3.17g  %3.17g]\nf =[%3.17g, %3.17g\nFL=%d %s]'],...
