...
% io = io2=1, тк движение - вправо и L2 правее(>) L1
...
if f1(1) ~= 0
   ...
   if f1(1)*f1(2) > 0 % io==1;    
      ...
      if ischar(x01)
         ...
         KIN = @KIN1;
         x01 = 2*sqrt(T21)-x01([2 1]); % отражение относ Т1
         f1  = HF(n,c,e11,e31,T21,V21,x01,ah1,bv1);
         if f1(1)*f1(2) > 0 % io==2, => исп только ветвь с KIN1
            x01 = KIN(n,c,e11,e31,T21,V21,x01,ah1,bn1,HF,Ve1,f1);
   ...
   if f2(1)*f2(2) > 0
      ...
      if ischar(x02)
         if io==2,  X = ['Solv51T io=2 x02=' x02];  return
            ...
            KIN = @KIN1;
            x02 = 2*sqrt(T22)-x02([2 1]); % отражение относ Т2
            f2  = HF(n,c,e12,e32,T22,V22,x02,ah,bv);
            if f2(1)*f2(2) > 0 % io==2, => исп только ветвь с KIN1
               x02 = KIN(n,c,e12,e32,T22,V22,x02,ah,bn,HF,Ve2,f2);
...
while i <= q 
  ...
  if FL == 1           % К след(i+1-му) корню  
     ...
     X = sprintf(['Solv51T: Корень не найден\nio=%d i=%d\n' ...  
         'x0=[%3.17g  %3.17g]\nf =[%3.17g, %3.17g\nFL=%d %s]'],...
