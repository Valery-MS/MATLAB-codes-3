...
function X = Solv51(nT,n,c,io,hL,L1, q,x01,f1,x02,f2,e1v,e3v,V2v,T2v) 
...
if f1(1) ~= 0
   ...
   if f1(1)*f1(2) > 0  % тк Е-корня здесь нет, то невып условия=>H-корень
      x01 = KIN( n,c,e11,e31,T21,V21,x01,ah,bn,HF,Ve1,f1); 
      if ischar(x01), X=sprintf('Solv51 nT=%d io=%d i=1/n%s',nT,io,x01);end        
   end
end    
...
if f2(1) ~= 0      
   ...
   if f2(1)*f2(2) > 0
      ...
      if ischar(x02), X=sprintf('Solv51 nT=%d io=%d i=2/n%s',nT,io,x02);end  
   end
end
...
while i <= q 
  ...
  if Xp == x0ep,  x0 = [x0e-AP;  x0e+AP];   %min(x0e+AP,Ve)]; 
     ...
     AP2 = AP+AP;   
     if dX > 0,  x0 = [x0e-AP/2; min(x0e+AP2,Ve)];
  ...
  if FL == 1           % К след(i+1-му) корню  
     ...
     X = sprintf(['Solv51: Корень не найден\nio=%d i=%d\n' ...  
         'x0=[%3.17g  %3.17g]\nf =[%3.17g, %3.17g\nFL=%d %s]'],...
